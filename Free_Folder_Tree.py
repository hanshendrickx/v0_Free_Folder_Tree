...
Usage:
    ...
    Beautiful:  python Free_Folder_Tree.py . --style artisanal --icons artisanal
    Perfect:    python Free_Folder_Tree.py . --style artisanal --icons artisanal --max-files 5 --depth 3
"""
...
class FolderTreeGenerator:
    ...
    def __init__(self):
        ...
        self.tree_chars = {
            'simple': {'branch': '├── ', 'last': '└── ', 'pipe': '│   ', 'space': '    '},
            'artisanal': {'branch': '├─ ', 'last': '└─ ', 'pipe': '│  ', 'space': '   '},
    ...
    def get_file_icon(self, file_path: Path) -> str:
        ...
        icon_map = {
            ...
            '.exe': 'executable', '.bat': 'executable', '.sh': 'executable',
        }
    ...
    def generate_tree_structure(self, 
                              ...
                              style: str = 'simple',
                              use_icons: bool = False) -> Tuple[List[str], Dict]:
        ...
        def traverse(path: Path, prefix: str = "", depth: int = 0, is_last: bool = True):
            if depth > max_depth:
                return
            ...
            try:
                ...
                size_info = f" ({self.get_file_size(path)})" if show_sizes and path.is_file() else ""
                ...
                if path.is_dir() and depth < max_depth:
                    try:
                        ...
                        children = [c for c in children if not self.should_exclude(c, exclude_patterns)]
                        ...
                        for i, child in enumerate(children):
                            ...
                            child_prefix = prefix + (chars['space'] if is_last else chars['pipe'])
                            traverse(child, child_prefix, depth + 1, child_is_last)
                    
                    except PermissionError:
                        lines.append(f"{prefix}{chars['space']}{chars['last']}[Permission Denied]")
                    except Exception as e:
                        lines.append(f"{prefix}{chars['space']}{chars['last']}[Error: {str(e)[:50]}]")
            
            except Exception as e:
                lines.append(f"{prefix}{chars['last']}[Error accessing {path.name}: {str(e)[:30]}]")
        
        # Start traversal
        root_line = f"{self.get_file_icon(root_path) if use_icons else ''}{root_path.name}/"
        lines.append(root_line)
        stats['folders'] += 1
        
        try:
            children = sorted(root_path.iterdir(), key=lambda x: (x.is_file(), x.name.lower()))
            if not show_hidden:
                children = [c for c in children if not c.name.startswith('.')]
            children = [c for c in children if not self.should_exclude(c, exclude_patterns)]
    ...
    def save_text_output(self, lines: List[str], stats: Dict, output_path: Path, root_path: Path):
    ...
    def save_png_output(self, lines: List[str], stats: Dict, output_path: Path, root_path: Path):
        ...
        line_height = 20
        char_width = 10
        max_width = max(len(line) for line in lines) * char_width + 100
        ...
        # Draw header
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        draw.text((20, 20), "FREE PROFESSIONAL FOLDER TREE", fill='black', font=font)
        draw.text((20, 45), f"Generated: {timestamp}", fill='gray', font=font)
        draw.text((20, 70), f"Root: {root_path.name}", fill='gray', font=font)
        draw.text((20, 95), f"Stats: {stats['folders']} folders, {stats['files']} files", fill='gray', font=font)
    ...
    def save_svg_output(self, lines: List[str], stats: Dict, output_path: Path, root_path: Path):
        """Save tree as SVG"""
        ...
        # Add header
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        dwg.add(dwg.text("FREE PROFESSIONAL FOLDER TREE", insert=(20, 30), class_="header"))
        ...
        dwg.add(dwg.text(f"Stats: {stats['folders']} folders, {stats['files']} files", insert=(20, 90), class_="info"))
    ...
    def save_pdf_output(self, lines: List[str], stats: Dict, output_path: Path, root_path: Path):
        """Save tree as PDF"""
        ...
        c.drawString(50, height - 100, f"Statistics: {stats['folders']} folders, {stats['files']} files")
    ...
    def generate_and_save(self, root_path: str, **kwargs):
        ...
        formats = kwargs.get('formats', ['txt'])
...
Complexity Levels:
  ...
  Perfect:    All features enabled, professional output
    ...
    parser.add_argument('path', help='Directory path to generate tree for')
    ...
    parser.add_argument('--style', choices=['simple', 'artisanal', 'minimal'], 
    ...
    parser.add_argument('--formats', nargs='+', choices=['txt', 'png', 'svg', 'pdf'],
                       default=['txt'], help='Output formats')
